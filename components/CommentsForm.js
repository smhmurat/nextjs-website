    ...
    <div className='bg-white shadow-lg rounded-lg p-8 pb-12 mb-8'>
      ...
      <div className='grid grid-cols-1 gap-4 mb-4'>
        <div>
          ...
          <label className='text-gray-500 cursor-pointer ml-2' htmlFor='storeData'>Daha sonraki yorumlar için isim ve mail bilgilerimi kaydet.</label>
        </div>
      </div>
      {error && <p className='text-xs text-brand-color'>Tüm alanları doldurmalısınız.</p>}
      <div className='mt-8'>
        <button type='button' onClick={handleCommentSubmission} className='transition duration-500 transform hover:-translate-y-1 inline-block bg-primary-color text-white text-lg font-medium rounded-full px-8 py-3 cursor-pointer'>Gönder</button>
        {showSuccessMessage && <span className='text-xl float-right font-semibold mt-3 text-green-500'>Yorum başarıyla gönderildi.</span>}
      </div>
    </div>
  )
}
